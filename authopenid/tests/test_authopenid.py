...
from authopenid.api import AuthenticationFailed

class TestAuthOpenIdPlugin(unittest.TestCase):
    def setUp(self):
    ...
    def get_plugin(self):
        ...
        return AuthOpenIdPlugin(self.env)
    ...
    def assert_process_request_shows_login_form(self, req):
        plugin = self.get_plugin()
        tmpl, data, ctype = plugin.process_request(req)
        self.assertEquals(tmpl, 'openid_login.html')
    ...
    def test_get_navigation_items_when_logged_in(self):
    ...
    def test_login_sets_start_page_from_referer(self):
        req = self.make_request(path_info='/openid/login',
                                args={'referer': 'foo'})
        plugin = self.get_plugin()
    ...
    def test_login_leaves_start_page_if_no_referer(self):
        req = self.make_request(path_info='/openid/login')
        plugin = self.get_plugin()
    ...
    def test_login_with_fancy_selector(self):
        req = self.make_request(path_info='/openid/login')
        plugin = self.get_plugin()
    ...
    def test_login_GET_with_default_openid(self):
    ...
    def test_login_POST_with_no_identifier(self):
        req = self.make_request(path_info='/openid/login', method='POST')
        self.assert_process_request_shows_login_form(req)
        self.assertIn("Enter an OpenID identifier", req.chrome['warnings'])


    def test_login_POST_discovery_failure(self):
        req = self.make_request(path_info='/openid/login', method='POST',
                                args={'openid_identifier': '=id'})
        plugin = self.get_plugin()
        with patch.object(plugin.__class__, 'openid_consumer') \
    ...
    def test_response_logs_user_in(self):
    ...
    def test_response_registers_user(self):
        req = self.make_request(path_info='/openid/response')
        plugin = self.get_plugin()
        with patch.multiple(plugin.__class__,
                            openid_consumer=DEFAULT,
    ...
    def test_response_redirects_to_login_page_on_failure(self):
        req = self.make_request(path_info='/openid/response')
        plugin = self.get_plugin()
        with patch.object(plugin.__class__, 'openid_consumer') \
                 as openid_consumer:
    ...
    def test_get_start_page_default(self):
        req = self.make_request()
        plugin = self.get_plugin()
        self.assertEquals(plugin.get_start_page(req), self.env.abs_href())

    def test_get_start_page_does_not_redirect_to_self(self):
        req = self.make_request()
        plugin = self.get_plugin()
        plugin.get_session(req).update(start_page='/openid/subpage')
        self.assertEquals(plugin.get_start_page(req), self.env.abs_href())

    def test_get_start_page_clears_session(self):
        req = self.make_request()
        plugin = self.get_plugin()
    ...
    def test_get_start_page_preserves_session(self):
        req = self.make_request()
        plugin = self.get_plugin()
        plugin.get_session(req).update(key='value')
